...
9. On macOS, rapidly resize a recording region, confirm window highlights use rounded corners and select the frontmost window, drag the selector controls from any non-interactive panel surface, and verify the selector, countdown, and HUD can be captured by another app while Captures excludes them from its own output.
...
11. Open a 1140×692 source in the editor and verify Fit and 100% previews, toolbar play/pause, the 12-frame filmstrip, playhead, trim handles, crop overlay, conditional audio controls, and millisecond labels stay synchronized.
12. Save an untouched MP4 with **Preserve quality** and verify it is byte-identical. Verify audio-only changes preserve the video stream, visual edits use the high-quality H.264 path, existing filenames are rejected, strict maximum-size output does not exceed its selected KB, MB, or GB ceiling, changing the destination saves into that folder, and a successful save opens Finder with the exact final size shown.
