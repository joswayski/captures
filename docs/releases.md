...
11. Verify the compact two-row recording HUD has no native shadow, shows unclipped tooltips and the faded **Controls aren’t recorded** note, labels its active state **Recording**, responds on hover, drags from non-interactive background space, hides completely, and returns when Captures is opened from the macOS Dock. Confirm Restart requires explicit native confirmation.
...
13. Save an untouched MP4 with **Preserve quality** and verify the original is updated without a quality change. Verify audio-only changes preserve the video stream, visual edits use the high-quality H.264 path, a failed edit leaves the original intact, **Save a copy** unlocks filename and destination editing while rejecting collisions, strict maximum-size output does not exceed its selected KB, MB, or GB ceiling, and a successful save opens Finder with the measured final size and a **Show in Folder** action shown.
