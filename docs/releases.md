...
13. Record a Retina region, window, and display with **Original** resolution and verify their masters use physical rather than logical pixels. Confirm cursor movement remains visible, **Show clicks** forces the cursor on and renders click feedback, and an untouched **Preserve quality** save updates the original without re-encoding. Verify audio-only changes preserve the video stream, visual edits use the high-quality H.264 path, a failed edit leaves the original intact, **Save as copy** unlocks filename and destination editing while rejecting collisions, strict maximum-size output does not exceed its selected KB, MB, or GB ceiling, and a successful save opens Finder with the measured final size and a **Show in Folder** action shown.
