...
10. Record for at least 10 seconds on a secondary display. Confirm the single full-display countdown fades in and out there, Escape cancels it, the start chime plays only when recording begins and is absent from recorded audio, and finalized duration is within 250 ms or 5% of wall-clock time, whichever is larger.
11. Verify the recording HUD has no native shadow, shows unclipped tooltips, labels its active state **Recording**, collapses and expands around its current position, and can be brought forward from the macOS Dock.
12. Open a 1140×692 source in the editor and verify Fit and 100% previews, toolbar play/pause, the 12-frame filmstrip, playhead, trim handles, crop overlay, labeled final video size, conditional audio controls, and millisecond labels stay synchronized.
13. Save an untouched MP4 with **Preserve quality** and verify it is byte-identical. Verify audio-only changes preserve the video stream, visual edits use the high-quality H.264 path, existing filenames are rejected, strict maximum-size output does not exceed its selected KB, MB, or GB ceiling, changing the destination saves into that folder, and a successful save opens Finder with the measured final size shown.
14. Delete a quick-access preview and confirm its native window becomes click-through immediately while the particle animation finishes.
15. Confirm the packaged FFmpeg/ffprobe sidecars run on a clean Mac and the release includes every source and compliance asset listed in `docs/media-sidecars.md`.
