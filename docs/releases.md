...
failure. Users can also download the `.dmg`, Windows setup, `.deb`, or AppImage
from captur.es or the README and install over the current copy. That recovery path
does not depend on the in-app updater working; settings and capture history stay.
