...
| Recording workflow | Pause/resume/restart/mute/stop/discard, Hide/Show, passive region guide, screenshots during recording, ready/saved notices; AppKit HUD microphone meter; both hosts provide frame scrubbing, retained full-source thumbnail timelines, graphical/numeric trim, graphical/numeric crop, display-only Fit/100%, preset/custom output size, track volume/mute/mono, selectable GIF cadence, quality-mapped palettes and maximum width, Play/Pause (silent by default), opt-in Loop and accepted-mix Sound preview, and MP4/GIF save-new-copy | Windows/Linux meter and device-change parity; physical recording/audio acceptance |
...
not a physical-device acceptance result. AppKit uses the existing serialized session
request for one bounded sample at a time only while its HUD is visible, running,
and unmuted. The fixed-glass meter clears on busy/paused/muted/hidden/terminal
states; delayed reads cannot revive a previous take. Windows/Linux host meter
integration and physical microphone/device verification remain open.
