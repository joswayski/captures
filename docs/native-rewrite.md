...
AppKit accepts GIF/MP4/WebM in its live serialized queue through `--open-media`
or its `--open-image` alias and file-open callbacks. It waits for settings, History,
frame and thumbnail settlement before advancing, focuses canonical active
recordings without losing staged work, and refuses unsafe editor switches.
External references cannot use Replace original; physical opening remains unverified.
...
the backend operation. The Windows/Linux host still opens only PNG/JPEG/WebP until
its separate file-open wiring lands.
