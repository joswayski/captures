...
- Edit screenshots with inline text, shapes, bendable arrows, drawing, crop, resize, zoom, and image layers that snap to edges while moving or resizing. Start annotations outside the canvas or release past an edge to expand it; empty chrome around the canvas deselects. Dropped images can snap to a layer edge or stack on top. Trim edges shrinks the canvas to the bounds of visible layers.
