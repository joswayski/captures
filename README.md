...
- Built-in screenshot editor — text, shapes (drag the stem above a selected shape to rotate it; hold Shift for 15°), drawing with optional drop shadows, crop (drag from outside the canvas to reach an edge; hold Shift to lock aspect), layers that hang off the canvas stay clipped until you expand, erase to transparent; unsaved edits restore when you reopen
