...
- Edit screenshots with inline text, shapes, multi-point bendable lines and arrows, drawing, crop, resize, zoom, and image layers that snap to edges while moving or resizing. After placing a shape, subtle handles stay available so you can edit it without switching tools; lines and arrows start with three curve dots, and you can double-click the path to add as many more as needed. Start annotations outside the canvas or release past an edge to expand it; empty chrome around the canvas deselects. Dropped images can snap to a layer edge or stack on top. Each layer row has lock and visibility toggles plus a ⋯ menu for stack order, merge down, merge visible, flatten, duplicate, and delete (each action is labeled with a short description). Trim edges shrinks the canvas to the bounds of visible layers. Remove background clears matching colors (magic wand) or lets you erase and restore alpha on image layers—including the locked capture—then export transparent PNG or WebP.
