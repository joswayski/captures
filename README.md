...
only with `--live`. AppKit additionally accepts `--open-media PATH` in the same
ordered queue; either spelling opens PNG, JPEG, WebP, GIF, MP4 and WebM there.
AppKit also handles macOS file-open requests while running. Windows/Linux still
accept PNG, JPEG and WebP only. Still images become local History-backed
screenshot editor sessions; unsupported files report an error without stopping
later paths. Reopening an already-open source focuses its editor without replacing
unsaved edits. Reopening a closed source refreshes the same History ID without
changing the source file; if it has a saved draft, restore or explicitly discard
that draft from History first.
...
AppKit GIF/video sources become History-backed recording editor references;
the source remains external and Replace original is unavailable for them.
Windows/Linux single-instance forwarding and GIF/video host opening remain separate work;
TIFF is supported by in-editor import, not this external-open path.
The shared backend separately supports an additive `open_media` request for GIF,
MP4 and WebM History references. It validates the actual container, editor preview
