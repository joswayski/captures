...
use sctk::reexports::client::{
    protocol::{wl_data_device::WlDataDevice, wl_data_device_manager::DndAction,
               wl_data_source::WlDataSource, wl_surface::WlSurface},
    Connection, Proxy, QueueHandle,
};

use super::{make_wid, state::WinitState};
...
pub struct OutboundDrag {
    ...
    pub own_drop: bool,
    pub finished: Option<Box<dyn FnOnce(bool, bool) + Send>>,
}
...
impl WinitState {
    fn finish_outbound_drag(&mut self, source: &WlDataSource, accepted: bool) {
        if self.outbound_drag.as_ref().is_none_or(|drag| drag.source.inner() != source) {
            return;
        }
        if let Some(mut drag) = self.outbound_drag.take() {
            let own = drag.own_drop;
            if let Some(finished) = drag.finished.take() {
                finished(accepted && drag.dropped && drag.action.contains(DndAction::Copy), own);
            }
        }
    }
}
...
impl DataSourceHandler for WinitState {
    ...
    fn send_request(&mut self, _: &Connection, _: &QueueHandle<Self>, source: &WlDataSource,
                    mime: String, mut fd: WritePipe) {
        if mime != "text/uri-list" { return; }
        if let Some(drag) = self.outbound_drag.as_ref().filter(|drag| drag.source.inner() == source) {
            let uri = drag.uri.clone();
            // The receiver may read late; never block the compositor event loop.
            std::thread::spawn(move || { let _ = fd.write_all(&uri); });
        }
    }
    ...
    fn dnd_dropped(&mut self, _: &Connection, _: &QueueHandle<Self>, source: &WlDataSource) {
        if let Some(drag) = self.outbound_drag.as_mut().filter(|drag| drag.source.inner() == source) {
            drag.dropped = true;
    ...
    fn action(&mut self, _: &Connection, _: &QueueHandle<Self>, source: &WlDataSource,
              action: DndAction) {
        if let Some(drag) = self.outbound_drag.as_mut().filter(|drag| drag.source.inner() == source) {
            drag.action = action;
        }
...
impl DataDeviceHandler for WinitState {
    fn enter(&mut self, _: &Connection, _: &QueueHandle<Self>, device: &WlDataDevice,
             _: f64, _: f64, surface: &WlSurface) {
        let own = self.windows.borrow().contains_key(&make_wid(surface));
        if let Some(drag) = self.outbound_drag.as_mut() {
            drag.over_own_window = own;
            if own {
                if let Some(offer) = device.data().and_then(|data: &sctk::data_device_manager::data_device::DataDeviceData| data.drag_offer()) {
    ...
    fn leave(&mut self, _: &Connection, _: &QueueHandle<Self>, _: &WlDataDevice) {
        if let Some(drag) = self.outbound_drag.as_mut() { drag.over_own_window = false; }
    }
    fn motion(&mut self, _: &Connection, _: &QueueHandle<Self>, _: &WlDataDevice, _: f64, _: f64) {}
    fn selection(&mut self, _: &Connection, _: &QueueHandle<Self>, _: &WlDataDevice) {}
    fn drop_performed(&mut self, _: &Connection, _: &QueueHandle<Self>, device: &WlDataDevice) {
        if self.outbound_drag.as_ref().is_some_and(|drag| drag.over_own_window) {
            if let Some(offer) = device.data().and_then(|data: &sctk::data_device_manager::data_device::DataDeviceData| data.drag_offer()) {
                offer.finish();
            }
        }
