...
impl ActiveEventLoop {
    pub fn start_wayland_file_drag(
        ...
        match self {
            #[cfg(wayland_platform)]
            ActiveEventLoop::Wayland(loop_) => loop_.start_file_drag(source.0, path, finished),
            #[cfg(x11_platform)]
            ActiveEventLoop::X(_) => Err("not a Wayland event loop"),
        }
    }
    ...
    pub fn start_x11_file_drag(
        ...
        match self {
            #[cfg(x11_platform)]
            ActiveEventLoop::X(loop_) => loop_.start_file_drag(source.0, path, finished),
            #[cfg(wayland_platform)]
            ActiveEventLoop::Wayland(_) => Err("not an X11 event loop"),
        }
    }
