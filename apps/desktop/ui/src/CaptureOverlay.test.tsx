...
describe("CaptureOverlay guidance", () => {
  ...
  it("attaches late window targets without hiding a revealed overlay", async () => {
    let captureSessionReady: ((event: { payload: ActiveSession }) => void) | null = null;
    vi.mocked(listen).mockImplementation(async (event, handler) => {
      if (event === "capture-session-ready") {
        captureSessionReady = handler as (event: { payload: ActiveSession }) => void;
      }
      return () => undefined;
    });
    ...
    expect(container.querySelector(".window-target")).toBeNull();

    await waitFor(() => {
      expect(captureSessionReady).not.toBeNull();
    });
    await act(async () => {
      captureSessionReady?.({
        payload: {
