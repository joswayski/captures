...
impl From<ExportEstimate> for RecordingExportEstimate {
    fn from(estimate: ExportEstimate) -> Self {
        Self {
...
pub async fn estimate_recording_export(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        toolchain
            .estimate_export_size(&input, &edit, &export, &cancel)
            .map(RecordingExportEstimate::from)
    })
    .await
    .map_err(|error| error.to_string())?
    .map_err(|error| error.to_string())
