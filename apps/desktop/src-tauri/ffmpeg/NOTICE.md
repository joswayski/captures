...
The bundled configurations are LGPL-only: GPL, nonfree, version-3-only,
network, and autodetected external components are disabled. The system zlib is
enabled explicitly for PNG posters. H.264 export uses Apple's VideoToolbox
encoder on macOS and Captures' in-process OpenH264 encoder on Windows and Linux;
