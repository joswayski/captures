...
fn button(text: &str, tip: &str) -> gtk::Button {
    ...
    if let Some(accessible) = b.accessible() {
...
fn clear(container: &gtk::Container) {
...
fn open_impl(
    ...
    if restored.is_some() {
        ...
        banner.content_area().add(&gtk::Label::new(Some(
    ...
    let shape_popover = gtk::Popover::new(Some(&shape_button));
    shape_popover
        .style_context()
    ...
    let sidebar_shell = gtk::ScrolledWindow::new(gtk::Adjustment::NONE, gtk::Adjustment::NONE);
    ...
    let properties_scroll = gtk::ScrolledWindow::new(gtk::Adjustment::NONE, gtk::Adjustment::NONE);
    ...
    let quality = gtk::Scale::with_range(gtk::Orientation::Horizontal, 55., 98., 1.);
    quality.set_value(92.);
    quality.set_size_request(90, -1);
    quality.set_tooltip_text(Some("Compression quality"));
    ...
    export_settings.pack_start(&field_label("Comparison", &compare), false, false, 0);
    ...
    }
    {
        ...
        copy.connect_clicked(move |_| match render(&s.borrow().doc) {
            Ok(image) => {
                gtk::Clipboard::get(&gdk::SELECTION_CLIPBOARD).set_image(&ui::pixbuf(&image));
...
fn install_editor_css() {
    ...
    let _ = provider.load_from_data(br#"
...
fn setup_canvas(
...
) {
    {
        ...
        area.connect_draw(move |a, c| {
            ...
            c.set_source_rgba(background.red(), background.green(), background.blue(), 1.);
            let _ = c.paint();
...
fn setup_sidebar(
    ...
    *refresh_cb.borrow_mut() = Some(Box::new(move || {
        clear(&ls.clone().upcast::<gtk::Container>());
        clear(&ps.clone().upcast::<gtk::Container>());
        let state = s.borrow();
        ...
        for index in (0..state.doc.layers.len()).rev() {
            ...
            if let Some(accessible) = select.accessible() {
                accessible.set_name("Rename layer");
            }
            let eye = icon_button(
                if l.visible {
                    "Hide layer"
            ...
            );
            {
                ...
                let a = a.clone();
                let r = r.clone();
                row.connect_drag_data_received(move |row, context, _, y, data, _, time| {
                    let Some(moved) = data.text().and_then(|value| value.parse::<usize>().ok())
                    else {
                    ...
                    checkpoint(&mut s);
                    let visual_above = y < row.allocation().height() / 2;
        ...
        if state.tool == Tool::RemoveBg {
            ...
            if state.erase_mode == EraseMode::Wand {
                ...
                if let Some(accessible) = tolerance.accessible() {
                    accessible.set_name("Color tolerance");
                }
                let s2 = s.clone();
                ...
                if let Some(accessible) = size.accessible() {
                    accessible.set_name("Brush size");
                }
                let s2 = s.clone();
                ...
                if let Some(accessible) = softness.accessible() {
                    accessible.set_name("Brush softness");
                }
                let s2 = s.clone();
            ...
            let s2 = s.clone();
            picker.connect_color_set(move |p| s2.borrow_mut().color = color(p.rgba()));
            style.pack_start(&picker, false, false, 0);
...
fn setup_keys(
...
) {
    {
        ...
        window.connect_key_release_event(move |_, e| {
            if e.keyval() == gdk::keys::constants::space {
    ...
    window.connect_key_press_event(move |_, e| {
        ...
        if ctrl && (e.keyval() == gdk::keys::constants::z || e.keyval() == gdk::keys::constants::y)
        {
            undo(
                &mut s,
                e.keyval() == gdk::keys::constants::y
        ...
        match e.keyval() {
            ...
            gdk::keys::constants::o => s.tool = Tool::Ellipse,
            gdk::keys::constants::a => s.tool = Tool::Arrow,
            gdk::keys::constants::l => s.tool = Tool::Line,
            gdk::keys::constants::p => s.tool = Tool::Pen,
            gdk::keys::constants::b => s.tool = Tool::RemoveBg,
...
fn selected_quality(
    ...
    match mode.active() {
        Some(1) => ExportQuality::Compress(quality.value() as u8),
        Some(2) => {
