...
fn overlay(
    ...
    panel.set_visible_window(false);
    panel.add(&controls);
        ...
        area.connect_draw(move |_, cr| {
            ...
            if let Some(r) = *rect.borrow() {
                ...
                if mode.get() == CaptureMode::Window {
                    cr.set_source_rgba(color.red(), color.green(), color.blue(), 0.14);
                    cr.rectangle(r.x, r.y, r.width, r.height);
                    let _ = cr.fill();
                }
                ...
                if mode.get() == CaptureMode::Region {
                    for (x, y) in [
                        ...
                        cr.set_source_rgba(ink.red(), ink.green(), ink.blue(), 1.);
                        cr.set_line_width(2.);
                        let _ = cr.stroke();
                    }
    ...
    });
    {
        ...
        window.connect_key_press_event(move |window, event| {
            match event.keyval() {
                gdk::keys::constants::Shift_L | gdk::keys::constants::Shift_R => shift_drag(true),
                ...
                gdk::keys::constants::r => buttons.borrow()[0].1.clicked(),
                gdk::keys::constants::w => buttons.borrow()[1].1.clicked(),
                gdk::keys::constants::f => buttons.borrow()[2].1.clicked(),
