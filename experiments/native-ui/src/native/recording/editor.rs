...
pub fn open(path: PathBuf, directory: PathBuf, on_saved: Rc<dyn Fn(PathBuf)>) {
    ...
    let page_scroll = gtk::ScrolledWindow::new(gtk::Adjustment::NONE, gtk::Adjustment::NONE);
    ...
    loop_toggle.set_image(Some(&ui::icon("loop", 16)));
    loop_toggle.set_always_show_image(true);
    ...
    let fit = gtk::RadioButton::with_label("Fit");
    let actual = gtk::RadioButton::with_label_from_widget(&fit, "100%");
    fit.set_active(true);
    fit.set_mode(false);
    actual.set_mode(false);
    ...
    let preview_events = gtk::EventBox::new();
    preview_events.set_visible_window(false);
    preview_events.add_events(
    ...
    comparison_handle.set_relief(gtk::ReliefStyle::None);
    comparison_handle
        .style_context()
    ...
    let comparison_gesture = gtk::GestureDrag::new(&comparison_handle);
    ...
    comparison_handle.connect_key_press_event({
        ...
        move |_, event| {
            let next = match event.keyval() {
                gtk::gdk::keys::constants::Left => Some(split.get() - 0.02),
                gtk::gdk::keys::constants::Right => Some(split.get() + 0.02),
                gtk::gdk::keys::constants::Home => Some(0.06),
                gtk::gdk::keys::constants::End => Some(0.94),
                _ => None,
            };
    ...
    let play_overlay = gtk::EventBox::new();
    ...
    let preview_scroll = gtk::ScrolledWindow::new(gtk::Adjustment::NONE, gtk::Adjustment::NONE);
    ...
    timeline_interaction.add_events(
        gtk::gdk::EventMask::BUTTON_PRESS_MASK
            | gtk::gdk::EventMask::BUTTON_RELEASE_MASK
            | gtk::gdk::EventMask::POINTER_MOTION_MASK,
    );
    ...
    crop_overlay.connect_draw({
        ...
        move |area, context| {
            ...
            context.set_source_rgba(accent.red(), accent.green(), accent.blue(), 0.98);
    ...
    timeline_interaction.connect_draw({
        ...
        move |area, context| {
            ...
            let accent = area
                ...
                .lookup_color("captures_accent")
                .unwrap_or_else(|| gtk::gdk::RGBA::new(0.10, 0.58, 0.96, 1.0));
            context.set_source_rgba(accent.red(), accent.green(), accent.blue(), 1.0);
    ...
    timeline_interaction.connect_key_press_event({
        ...
        move |area, event| {
            ...
            if event.keyval() == gtk::gdk::keys::constants::Tab
                || event.keyval() == gtk::gdk::keys::constants::ISO_Left_Tab
            {
            ...
            let next = match event.keyval() {
                gtk::gdk::keys::constants::Left | gtk::gdk::keys::constants::Down => {
                    Some(current.saturating_sub(step))
                }
                gtk::gdk::keys::constants::Right | gtk::gdk::keys::constants::Up => {
                    Some(current.saturating_add(step))
                }
                gtk::gdk::keys::constants::Page_Down => Some(current.saturating_sub(large_step)),
                gtk::gdk::keys::constants::Page_Up => Some(current.saturating_add(large_step)),
                gtk::gdk::keys::constants::Home => Some(match target.get() {
                    TimelineDrag::End => start.value().max(0.0) as u64 + 1,
                    _ => 0,
                }),
                gtk::gdk::keys::constants::End => Some(match target.get() {
    ...
    choose_destination.connect_clicked({
        ...
        move |_| {
            ...
            let _ = dialog.set_current_folder(destination.borrow().as_path());
