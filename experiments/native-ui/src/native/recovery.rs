...
pub fn open(directory: PathBuf, saved: Rc<dyn Fn(PathBuf)>) {
    ...
    for draft in drafts {
        ...
        }
        {
            ...
            let dialog=gtk::MessageDialog::new(Some(&window),gtk::DialogFlags::MODAL,gtk::MessageType::Warning,gtk::ButtonsType::OkCancel,"Permanently discard this unfinished recording? Its unsaved segments cannot be recovered afterward.");
            let confirmed=dialog.run()==gtk::ResponseType::Ok;dialog.close();if !confirmed{return;}
            match recording::discard_recording_draft(&directory,&id){Ok(())=>row.hide(),Err(error)=>status.set_text(&error)}
        });
        }
    }
