use crate::{
    editor::{BlendMode, Document, Layer, Shape},
    geometry::{Point, Rect},
};
...
impl DocumentData {
    fn into_document(self, assets: &Path) -> Result<Document, String> {
        ...
        Ok(Document {
            id: super::editor::NEXT_DOCUMENT_ID.fetch_add(1, std::sync::atomic::Ordering::Relaxed),
            original,
            source,
...
mod tests {
    ...
    fn restart_restores_editable_raster_layers_without_reading_or_rewriting_source() {
        ...
        *pixels = Arc::new(erased);
        ...
        assert_eq!(restored.layers[0].opacity, 173);
        let dimensions = (restored.canvas_width, restored.canvas_height);
        let mut state = crate::state::AppState::default();
        state.edit_document_from(restored, Some(source_path.clone()), dimensions);
        assert_eq!(state.editor_source.as_deref(), Some(source_path.as_path()));
        assert!(!state.editor_save_as_new);
        assert!(crate::state::can_replace_editor_source(
