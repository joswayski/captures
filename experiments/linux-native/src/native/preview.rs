...
use crate::ui;
use gtk::{cairo, gdk, gdk_pixbuf::Pixbuf, gio, glib, prelude::*};
use image::RgbaImage;
use std::{
    cell::{Cell, RefCell},
    io::Cursor,
...
fn install_preview_css() {
    ...
    provider.load_from_data(include_str!("preview.css"));
...
impl Preview {
    pub fn with_callbacks(callbacks: PreviewCallbacks) -> Self {
        ...
        window.set_title(Some("Captures — Mini previews"));
        ...
        fixed.put(&pile_hit, CARD_X, STACK_EDGE);
        ...
        show_less_content.add_named(&preview_stack_icon(), Some("icon"));
        let show_less_label = gtk::Label::new(Some("Show less"));
        show_less_label
            .style_context()
            .add_class("preview-show-less-label");
        show_less_content.add_named(&show_less_label, Some("label"));
        ...
        fixed.put(&toolbar, CARD_X, f64::from(HEIGHT - 44));
        fixed.put(&newer, 147.0, 6.0);
        fixed.put(&older, 147.0, f64::from(HEIGHT - 28));
        ...
        }
        {
            ...
            this.0
                ...
                .connect_size_allocate(move |toolbar, allocation| {
                    ...
                    this.0.fixed.move_(toolbar, f64::from(x), f64::from(y));
                });
        }
        {
            let this = this.clone();
    ...
    pub fn add_with_metadata(&self, image: RgbaImage, path: PathBuf, metadata: PreviewMetadata) {
        ...
        self.0.fixed.put(&shell, CARD_X, STACK_EDGE);
    ...
    fn confirm_delete(&self, id: u64) {
        ...
        ui::confirm(
            &self.0.window,
            &format!("Delete {} permanently?", card_name(&path)),
            "Close keeps the saved file and Capture History entry.",
            "Delete",
            move || stack.exit(id, ExitKind::Delete, 0.),
        );
    }
    ...
    fn copy_card(&self, id: u64) -> bool {
        ...
        gdk::Display::default()
            .unwrap()
            .clipboard()
            .set_texture(&gdk::Texture::for_pixbuf(&image));
    ...
    fn finish_file_drag(&self, id: u64, context: &gdk::Drag) {
        if context.selected_action().is_empty() {
            if let Some(card) = self
                .0
                .state
                .borrow_mut()
    ...
    fn reflow(&self) {
        ...
        for (index, card) in state.cards.iter().enumerate() {
            ...
            if visible {
                self.0
                    .fixed
                    .move_(&card.shell, CARD_X, Self::card_y(&state, local).round());
            }
        }
        self.0
            .pile_hit
        ...
        self.0
            .fixed
            .move_(&self.0.toolbar, f64::from(toolbar_x), f64::from(toolbar_y));
        self.0.fixed.move_(&self.0.newer, 147.0, 6.0);
        self.0
            .fixed
            .move_(&self.0.older, 147.0, f64::from(HEIGHT - 28));
        drop(state);
    ...
    fn update_input_shape(&self) {
        ...
        if let Some(surface) = gtk::prelude::NativeExt::surface(&self.0.window)
            && let Ok(surface) = surface.downcast::<gdk4_x11::X11Surface>()
            && let Ok((connection, _)) = x11rb::connect(None)
        {
            use x11rb::{connection::Connection, protocol::shape::ConnectionExt};
            let rectangles = (0..input.num_rectangles())
                .map(|index| input.rectangle(index))
                .map(|rectangle| x11rb::protocol::xproto::Rectangle {
                    x: rectangle.x().clamp(i16::MIN.into(), i16::MAX.into()) as i16,
                    y: rectangle.y().clamp(i16::MIN.into(), i16::MAX.into()) as i16,
                    width: rectangle.width().clamp(0, u16::MAX.into()) as u16,
                    height: rectangle.height().clamp(0, u16::MAX.into()) as u16,
                })
                .collect::<Vec<_>>();
