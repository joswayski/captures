...
fn theme_swatch(accent: &str, signal: &str, custom: bool) -> gtk::DrawingArea {
    ...
    swatch.connect_draw(move |area, cr| {
        ...
        if custom {
            ...
            cr.set_source_rgba(
                f64::from(signal.red()),
                f64::from(signal.green()),
                f64::from(signal.blue()),
                1.0,
            );
...
pub fn open(
    ...
    window.set_title(Some("Captures Preferences"));
    window.set_default_size(980, 720);
    window.set_position(());

    let provider = gtk::CssProvider::new();
    provider.load_from_data(include_str!("preferences.css"));
    ...
    let choose = ui::button("Choose…");
    {
        ...
        choose.connect_clicked(move |_| {
            ...
            ui::choose_folder(&parent, "Save captures to", &initial, move |path| {
    ...
    for (title, value, assign) in [
        ...
        color.connect_color_set(move |button| {
            ...
            let hex = format!(
                "#{:02x}{:02x}{:02x}",
                (f64::from(rgba.red()) * 255.).round() as u8,
                (f64::from(rgba.green()) * 255.).round() as u8,
                (f64::from(rgba.blue()) * 255.).round() as u8
            );
            if assign == 0 {
