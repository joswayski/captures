...
impl App {
    ...
    fn new(application: &gtk::Application) -> Rc<Self> {
        ...
        window.set_title(Some(if cfg!(target_os = "windows") {
    ...
    fn action(self: &Rc<Self>, action: Action) {
        match action {
            ...
            Action::Open => {
                let app = self.clone();
                ui::open_file(&self.window, move |path| app.open(path));
            }
                ...
                match result {
                    Ok((image, path, config)) => {
                        ...
                        if config.auto_copy_to_clipboard {
                            ...
                            clipboard.set_texture(&ui::texture(&image));
                        }
...
fn countdown(seconds: u8, done: Rc<dyn Fn()>, cancelled: Rc<dyn Fn()>) {
    ...
    let window = gtk::Window::new();
    window.set_title(Some("Captures countdown"));
...
fn new_canvas(parent: &gtk::Window, directory: PathBuf, saved: Rc<dyn Fn(PathBuf)>) {
    ...
    if dialog.run() == gtk::ResponseType::Accept {
        ...
        let image = image::RgbaImage::from_pixel(
            ...
            image::Rgba([
                (f64::from(c.red()) * 255.) as u8,
                (f64::from(c.green()) * 255.) as u8,
                (f64::from(c.blue()) * 255.) as u8,
