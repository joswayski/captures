...
- Footer review: compact MP4/GIF picker and Save action, inset filename heading, highlighted directory picker link, and Before/After size badges replace the persistent comparison-ready message. Before reports original bytes; After explicitly estimates the selected duration, not the untrimmed source. Light/dark, long-directory cancellation, geometry bounds and full recording actions pass. Fresh photo-editor, Preferences and History checks pass; History's Delete all no longer stretches to the heading height.
