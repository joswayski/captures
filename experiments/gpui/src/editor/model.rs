//! Layer document and Cairo renderer adapted from Captures' GTK native editor.
//! The original implementation is Copyright Captures contributors and is used
//! here under this repository's Apache-2.0 license.
...
fn path(ctx: &cairo::Context, l: &Layer) {
    match &l.kind {
        ...
        LayerKind::Arrow(a, b) | LayerKind::Line(a, b) => {
            ctx.move_to(a.x, a.y);
            if let [control] = l.controls.as_slice() {
                ctx.curve_to(
                    a.x + 2. / 3. * (control.x - a.x),
                    a.y + 2. / 3. * (control.y - a.y),
                    b.x + 2. / 3. * (control.x - b.x),
                    b.y + 2. / 3. * (control.y - b.y),
                    b.x,
                    b.y,
                );
            } else if l.controls.is_empty() {
                ctx.line_to(b.x, b.y);
            } else {
                for control in &l.controls {
                    ctx.line_to(control.x, control.y);
                }
                ctx.line_to(b.x, b.y);
            }
        }
...
fn rounded_rectangle(ctx: &cairo::Context, width: f64, height: f64, radius: f64) {
    let radius = radius.clamp(0., width.min(height) / 2.);
    ctx.new_sub_path();
    ctx.arc(width - radius, radius, radius, -PI / 2., 0.);
...
pub fn draw_layer(ctx: &cairo::Context, l: &Layer) -> Result<(), String> {
    ...
    if let Some(shadow) = l.shadow {
        // Cairo has no portable image-filter API. Several translucent offset
        // passes form the same restrained contact-shadow pool without GTK.
        let steps = shadow.blur.clamp(0., 100.).ceil().min(16.) as i32;
        for y in -steps..=steps {
            for x in -steps..=steps {
                let distance = f64::from(x * x + y * y).sqrt();
                if distance > f64::from(steps) {
                    continue;
                }
                let alpha = shadow.color.3 * l.opacity * (1. - distance / f64::from(steps.max(1)))
                    / f64::from(steps.max(1));
                let _ = ctx.save();
                ctx.translate(
                    shadow.offset_x + f64::from(x),
                    shadow.offset_y + f64::from(y),
                );
                path(ctx, l);
                ctx.set_source_rgba(shadow.color.0, shadow.color.1, shadow.color.2, alpha);
                if matches!(
                    l.kind,
                    LayerKind::Rectangle
                        | LayerKind::Ellipse
    ...
    match &l.kind {
        ...
        LayerKind::Text(text) => {
            ...
            if let Some(background) = l.text_style.background {
                ctx.set_source_rgba(
                    background.0,
                    background.1,
                    background.2,
                    background.3 * l.opacity,
                );
                if l.text_style.rounded_background {
                    rounded_rectangle(ctx, l.frame.w, l.frame.h, l.frame.h * 0.22);
                } else {
                    ctx.rectangle(0., 0., l.frame.w, l.frame.h);
                }
                let _ = ctx.fill();
                ctx.set_source_rgba(l.color.0, l.color.1, l.color.2, l.color.3 * l.opacity);
            }
            ctx.select_font_face(
                &l.text_style.family,
                if l.text_style.italic {
                    cairo::FontSlant::Italic
                } else {
                    cairo::FontSlant::Normal
                },
                if l.text_style.bold {
            ...
            let extents = ctx.text_extents(text).map_err(|error| error.to_string())?;
            let x = match l.text_style.align.as_str() {
                "center" => (l.frame.w - extents.width()) / 2.,
                "right" => l.frame.w - extents.width(),
                _ => 0.,
            };
            ctx.move_to(x.max(0.), l.frame.h * 0.78);
            if l.text_style.outlined {
                ctx.text_path(text);
                ctx.set_line_width((l.frame.h * 0.06).max(1.));
                let _ = ctx.stroke();
            } else {
...
pub fn content_bounds(doc: &Document) -> Option<Rect> {
    doc.layers
        ...
        .filter(|l| l.visible)
        .map(|layer| {
            let mut bounds = layer_bounds(layer);
            if let Some(shadow) = layer.shadow.filter(|shadow| shadow.color.3 > 0.) {
                let pad = shadow.blur * 2.;
                let left = (pad - shadow.offset_x).max(0.);
                let top = (pad - shadow.offset_y).max(0.);
                let right = (pad + shadow.offset_x).max(0.);
                let bottom = (pad + shadow.offset_y).max(0.);
                bounds.x -= left;
                bounds.y -= top;
                bounds.w += left + right;
                bounds.h += top + bottom;
...
#[cfg(test)]
fn expand_to_content(doc: &mut Document, padding: f64) {
    let Some(r) = content_bounds(doc) else { return };
...
mod tests {
    ...
    #[test]
    fn rounded_text_background_leaves_transparent_corners() {
        let mut document = Document::transparent(100, 60);
        let index = document.add(
            LayerKind::Text("A".into()),
            Rect {
                x: 10.,
                y: 10.,
                w: 70.,
        ...
        document.layers[index].text_style.background = Some(Color(1., 0., 0., 1.));
        document.layers[index].text_style.rounded_background = true;
        let output = render(&document).unwrap();
        assert_eq!(output.get_pixel(10, 10).0[3], 0);
        assert_eq!(output.get_pixel(45, 10).0, [255, 0, 0, 255]);
        assert_eq!(output.get_pixel(45, 30).0[3], 255);
    }
    ...
    #[test]
    fn trim_preserves_asymmetric_drop_shadow_spill() {
        let mut document = Document::transparent(200, 160);
        let index = document.add(
            LayerKind::Rectangle,
            Rect {
                x: 50.,
        ...
        document.layers[index].shadow = Some(Shadow {
            color: Color(0., 0., 0., 0.5),
            blur: 6.,
            offset_x: -8.,
            offset_y: 10.,
        });
        trim_to_content(&mut document, 0.);
        assert_eq!((document.width, document.height), (54, 44));
        assert_eq!(
            (document.layers[0].frame.x, document.layers[0].frame.y),
            (20., 2.)
        );
    }
