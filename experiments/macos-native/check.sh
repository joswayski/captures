...
CARGO_BUILD_JOBS="${CARGO_BUILD_JOBS:-2}" cargo test --release --locked --manifest-path "$experiment/bridge/Cargo.toml"
CARGO_BUILD_JOBS="${CARGO_BUILD_JOBS:-2}" cargo clippy --release --locked --manifest-path "$experiment/bridge/Cargo.toml" --all-targets -- -D warnings
if [[ "$(uname -s)" == Darwin ]]; then
