...
- Treat automatic per-merge builds as experimental Previews. Do not publish a stable release until the maintainer explicitly starts that channel.
...
- Deliver the native rewrite in cross-platform feature slices, not a complete macOS app followed by ports. Follow `docs/native-rewrite.md`; record macOS, Windows, X11, and Wayland implementation/verification status in each slice's PRs. Stubs and unverified hosts do not complete a parity gate.
...
- Regular windows follow the light/dark/system appearance setting. Surfaces that float over the user's desktop (capture overlay, capture menu, recording controls, mini previews, saved/hidden recording notices, the post-update / launch tooltip) always use the fixed `--glass-*` media palette so they stay legible on any wallpaper. The update notice is a solid `--surface-raised` card in a transparent native window. The launch notice is a dark glass pill with a CSS triangle caret pointing at the tray or menu bar icon — not a rotated square.
