...
- Regular windows follow the light/dark/system appearance setting. Surfaces that float over the user's desktop (capture overlay, capture menu, recording controls, mini previews, saved/hidden recording notices) always use the fixed `--glass-*` media palette so they stay legible on any wallpaper. The update notice is a solid `--surface-raised` card in a transparent native window. The post-update / launch notice is a pill tooltip. Both use a triangular caret that points at the tray or menu bar icon.
