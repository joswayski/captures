...
/// Lock-free snapshot of one source-sample/DAC-time pair. The callback writes
/// the sequence odd around updates so readers never combine different pairs.
...
impl AudioPlaybackClock {
    ...
    fn publish(&self, source_frame: u64, valid_frames: u64, playback_at: Instant) {
        ...
        self.source_frame.store(source_frame, Ordering::Relaxed);
        self.valid_frames.store(valid_frames, Ordering::Relaxed);
        let nanos = playback_at
            .saturating_duration_since(self.origin)
            .as_nanos()
            .min(u128::from(u64::MAX)) as u64;
        self.playback_nanoseconds.store(nanos, Ordering::Relaxed);
    ...
    pub(crate) fn position_ms(&self, start_position_ms: u64) -> Option<u64> {
        ...
        loop {
            ...
            let source_frame = self.source_frame.load(Ordering::Relaxed);
            let valid_frames = self.valid_frames.load(Ordering::Relaxed);
            let playback_nanoseconds = self.playback_nanoseconds.load(Ordering::Relaxed);
    ...
    T: Sample + FromSample<f32> + Copy,
{
    ...
    for frame in &mut frames {
        if consumer.occupied_len() >= channel_count {
            for sample in frame {
...
mod tests {
    ...
    #[test]
    fn clock_maps_source_samples_to_predicted_playback_without_tearing() {
        let clock = AudioPlaybackClock::new(48_000);
        let playback_at = Instant::now() + Duration::from_millis(40);
        clock.publish(4_800, 480, playback_at);
        assert_eq!(clock.position_ms(1_000), Some(1_100));
        assert!(!clock.published_buffer_finished());
        thread::sleep(Duration::from_millis(60));
        assert_eq!(clock.position_ms(1_000), Some(1_110));
        assert!(clock.published_buffer_finished());
    }
